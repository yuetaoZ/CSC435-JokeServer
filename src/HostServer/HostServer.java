...
class AgentWorker extends Thread {
	...
	agentHolder parentAgentHolder; //maintains agentstate holding socket and state counter, will update to new agentHolder by create a new one
	...
	public void run() {
		...
		try {
			...
			//log a request
			System.out.println();
			System.out.println("Request line: " + inLine);	// print the request
			...
			if(inLine.indexOf("migrate") > -1) {
				...
				for(;;) {	// this loop will run until get the input "[Port= something]" 
...
class agentHolder {
	...
	ServerSocket sock;
	//basic agentState var, to keep the agent State for new agent Holder
	int agentState;
...
class AgentListener extends Thread {
	...
	public void run() {
		...
		try {
			...
			agenthold.agentState = agentState;	// for keep the agentState in another Holder when the server sock is accept.
...
public class HostServer {
	...
	public static void main(String[] a) throws IOException {
		...
		System.out.println("Yuetao's DIA Master receiver started at port 1565.");
